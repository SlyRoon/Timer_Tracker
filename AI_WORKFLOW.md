...
## Автономний режим AI/Codex

AI/Codex працює як автономний інженерний агент, а не як пасивний генератор коду.

### Дозволено самостійно
- Створювати нові git branches, якщо цього логічно вимагає задача або ізоляція змін.
- Перемикатися між branches, якщо це потрібно для виконання prompt.
- Виконувати `git fetch`, `git pull`, `git merge`, `git push` і пов'язані git-команди, якщо вони потрібні для задачі.
- Мержити гілки, коли користувач просить перенести зміни або коли це очевидний завершальний крок для поточного workflow.
- Запускати релевантні перевірки, tests, build, typecheck або інші локальні команди, якщо вони доречні для виконаних змін.
- Оновлювати документацію, конфіги і workflow-файли, якщо prompt змінює правила роботи або структуру проєкту.

### Як обирати git workflow
- Якщо користувач явно назвав branch, працюй із нею.
- Якщо branch не названа, визнач поточну branch через `git branch --show-current`.
- Для великої нової фічі створи окрему логічну branch.
- Для дрібної правки або follow-up можна працювати в поточній branch, якщо це не змішує непов'язані зміни.
- Не запитуй додаткового дозволу на звичайні git-операції, якщо їхня потреба випливає з prompt.
- Не роби destructive operations на кшталт `git reset --hard`, force-push або видалення branches без прямого запиту користувача.

### PROMPTS_LOG.md
- Логуй ключові етапи, важливі implementation prompts і значущі workflow-зміни.
- Якщо користувач прямо уточнює поведінку агента і каже **не логувати**, не створюй новий entry у `PROMPTS_LOG.md`.
- Для залогованих implementation entries зберігай `Original user prompt`:
  - дослівно, якщо prompt короткий;
  - summary + verbatim excerpt, якщо prompt довгий.
- Не замінюй реальний user prompt загальною фразою.

### Завершення задачі
Після змістовної задачі, яка змінює файли або стан git, AI/Codex самостійно:
1. перевіряє `git status`;
2. додає релевантні зміни в git;
3. створює логічний commit;
4. виконує push у відповідну branch, якщо це доречно для задачі;
5. показує commit, branch, push result і виконані перевірки.

Якщо commit, push, tests або build не вдалися:
- явно покажи помилку;
- вкажи, на якому кроці збій;
- дай точну команду для ручного повтору.
...
- оновити `PROMPTS_LOG.md`, якщо prompt є ключовим implementation/workflow етапом і користувач не сказав не логувати;
- самостійно виконати потрібні git-операції: branch, switch, merge, commit, push;
...
- Після змістовних змін Codex сам виконує `git status`, `git add`, `git commit` і, якщо доречно, `git push`.
- Codex може сам створювати branch, перемикатися між branches, робити merge/pull/push, якщо це логічно випливає з prompt.
- Працюй у поточній branch, якщо користувач не попросив інше і задача не потребує окремої branch.
- Поточну branch визначай через `git branch --show-current`, якщо вона не очевидна.
- Не завершуй задачу без спроби commit.
- Не завершуй задачу без спроби push, якщо задача передбачає публікацію змін або користувач просив push.
...
- Не використовуй destructive git operations без прямого запиту користувача.
...
### 10.1. Що логувати
- Логуються ключові implementation prompts, важливі етапи і значущі workflow-зміни.
- Не ведеться мікролог кожної дрібної внутрішньої правки.
- Якщо користувач прямо каже не логувати службову корекцію, не додавай новий entry у `PROMPTS_LOG.md`.
- Після кожного залогованого user prompt `PROMPTS_LOG.md` оновлюється автоматично, якщо є доступ до файлів.
...
8. Якщо задача потребує окремої branch, створи її самостійно з логічною назвою.
9. Після змін онови `PROMPTS_LOG.md`, якщо prompt потрібно логувати.
10. Виконай релевантні перевірки/tests/build, якщо вони доречні.
11. Виконай потрібні git-операції: add, commit, push, merge або branch switch.
12. Наприкінці дай короткий звіт, commit message, push result і наступний логічний крок.
...
- original user prompt записано в `PROMPTS_LOG.md`, якщо prompt логувався;
- зміни додано в git;
- commit створено або явно показано причину збою;
- push виконано або явно показано причину збою, якщо push доречний для задачі;
