...
## Обов’язкове git та prompts-log завершення кожного етапу

Після кожного виконаного user prompt AI/Codex зобов’язаний:

1. Оновити `PROMPTS_LOG.md`
2. Додати в лог:
   - назву етапу
   - branch
   - ключовий промпт
...
4. Не завершувати задачу без спроби push
5. Якщо push не вдався:
   - явно показати помилку
   - вказати, на якому кроці збій
   - дати точну команду для ручного повтору

### Правило для `Original user prompt`
У кожному новому entry в `PROMPTS_LOG.md` потрібно зберігати саме той user prompt, який запустив поточну задачу:
- дослівно, якщо він короткий
- condensed version + verbatim excerpt, якщо він довгий

Не можна замінювати реальний user prompt загальною фразою.
