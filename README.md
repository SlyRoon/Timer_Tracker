...
Зараз завершено **Етап 5 — Controllers + validators + middlewares**.

Стартовий scaffold, backend domain foundation, repository layer, service layer і HTTP layer baseline створено:
...
- routes wiring ще не реалізовано;
- feature HTTP endpoints ще не підключені в `app.ts`;
- Project CRUD HTTP endpoints ще не реалізовано;
- timer HTTP endpoints ще не реалізовано;
- today entries HTTP endpoints ще не реалізовано;
- task autocomplete HTTP endpoint ще не реалізовано;
- reports HTTP endpoints і CSV export ще не реалізовано;
